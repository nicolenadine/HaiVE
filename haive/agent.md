...
cli.py — Command-line interface for indexing, discovering, loading, executing tasks, and managing configuration
  ...
  config_delete (function) — 84-91 — Deletes a named configuration
  _check_git_on_path (function) — 96-102 — Verifies git is available on PATH
  _check_active_config (function) — 105-110 — Validates that an active config file can be loaded
  _resolve_milestone_id (function) — 113-123 — Resolves milestone ID from CLI arg or config fallback
  _preflight_checks (function) — 126-128 — Runs all preflight checks before command execution
  _print_dry_run_output (function) — 133-156 — Formats and prints orchestrator dry-run output
